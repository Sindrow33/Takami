...
class WebtoonFeedView @JvmOverloads constructor(
    ...
    private fun drawPage(canvas: Canvas, item: FeedItem.Page, top: Float, heightPx: Float) {
        ...
        if (bmp == null) {
            ...
            val placeholderColor = item.edgeColors?.top ?: android.graphics.Color.WHITE
            canvas.drawRect(0f, top, width.toFloat(), top + heightPx, Paint().apply { color = placeholderColor })
            return
        }
    ...
    private fun drawEndCap(canvas: Canvas, top: Float, heightPx: Float) {
        ...
        canvas.drawRect(0f, top, width.toFloat(), top + heightPx, Paint().apply { color = android.graphics.Color.WHITE })
    }
