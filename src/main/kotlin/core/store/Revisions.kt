...
class RevisionManager(
    ...
    /** Запоминает метрики до начала испытательного срока. */
    fun armBaseline(host: String, health: SourceHealth) {
        val book = store.book(host)
        val p = book.probation ?: return
        store.save(book.copy(probation = p.copy(baseline = Baseline(
            successRate = health.successRate,
            emaFill = health.emaFill,
            emaConfidence = health.emaConfidence,
            lastGoodCount = health.lastGoodCount,
