...
data class Match(
    ...
    val runnerUp: Double,
    /**
     * Сколько кандидатов набрали ровно тот же балл. В списке из
     * одинаковых карточек заголовки неразличимы по отпечатку — и это
     * не неоднозначность: любой из клонов равно годится как образец
     * для синтеза селектора. Без этой поправки ремонт каталога
     * с повторяющимися названиями не начинался вообще.
     */
    val clones: Int = 0,
) {
    ...
    /** Второй кандидат — тот же элемент в соседней карточке. */
    val isCloneTie: Boolean get() = clones > 0 && margin <= 1e-6

    val isConfident: Boolean get() =
        score >= Similarity.STRONG ||
            (score >= Similarity.ACCEPT && (margin >= Similarity.MARGIN || isCloneTie))
}
...
object SignatureSearch {
    ...
    fun bestMatch(sig: ElementSignature, candidates: List<Node>): Match? {
        ...
        for (n in candidates) {
            ...
            when {
                s > bestScore -> {
                    // Прежний лидер становится вторым — но если он равен новому
                    // с точностью до эпсилона, это клон, а не конкурент.
                    clones = if (bestScore > 0.0 && bestScore >= s - CLONE_EPS) clones + 1 else 0
                    runnerUp = bestScore; bestScore = s; best = n
                }
                s >= bestScore - CLONE_EPS -> clones++
                s > runnerUp -> runnerUp = s
            }
        }
        val node = best ?: return null
        return Match(node, bestScore, runnerUp, clones)
    }

    private const val CLONE_EPS = 1e-6
